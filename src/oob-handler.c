...
int select(int nfds, fd_set *readfds, fd_set *writefds,
    ...
    bool should_monitor_oob = false;
    if ((readfds && FD_ISSET(MONITORED_FD, readfds)) || (writefds && FD_ISSET(MONITORED_FD, writefds))) {
        should_monitor_oob = true;
    }
    ...
    if (should_monitor_oob) {
        ...
        if (ret > 0 && FD_ISSET(MONITORED_FD, &new_exceptfds)) {
            try_recv_oob_token(MONITORED_FD, 0);
            ret -= 1;
        }

        if (should_send_token) {
            LOG("[OOB-HANDLER] pid=%d select: sockfd=%d has_token, ret=%d\n",
                getpid(), MONITORED_FD, ret);
...
ssize_t send(int sockfd, const void *buf, size_t len, int flags) {
    ...
    if (sockfd == MONITORED_FD && !(flags & MSG_OOB)) {
        LOG("[OOB-HANDLER] pid=%d send: sockfd=%d, len=%zu, flags=0x%x\n",
